...
async def start(bot, message):
    await message.reply(
        ...
        "**I am your Personal MDiskPro Link Converter Bot 🤗, Made by @MDisk_Pro_Official💞 Send me a MDiskPRO Post to see the Magic 😅**")
    
@bot.on_message(filters.text & filters.private)
async def Doodstream_uploader(bot, message):
    new_string = str(message.text)
    conv = await message.reply("Wait Your Link Is In Process @MDisk_Pro_Official Check Karo✋")
...
async def Doodstream_up(link):
    ...
    realaurl = 'https://mdiskpro.com/v1/tp/cp'
...
async def remove_username(new_List):
    for i in new_List:
        if('@' in i or 't.me' in i or 'https:///abcd' in i or 'https:///123abcd' in i or 'telegra.ph' in i):
            new_List.remove(i)
    return new_List
